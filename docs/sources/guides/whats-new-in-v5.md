+++
title = "What's New in LogDisplayPlatform v5.0"
description = "Feature & improvement highlights for LogDisplayPlatform v5.0"
keywords = ["logdisplayplatform", "new", "documentation", "5.0"]
type = "docs"
[menu.docs]
name = "Version 5.0"
identifier = "v5.0"
...
# What's New in LogDisplayPlatform v5.0

This is the most substantial update that LogDisplayPlatform has ever seen. This article will detail the major new features and enhancements.
...
- [Permissions]({{< relref "#dashboard-folders" >}}) on folders and dashboards helps manage larger LogDisplayPlatform installations.
...
above or on the [play site](http://play.logdisplayplatform.org). All your existing dashboards will automatically migrate to the
new position system and look close to identical. The new panel position makes dashboards saved in v5.0 incompatible
with older versions of LogDisplayPlatform.
...
Almost every page has seen significant UX improvements. All pages (except dashboard pages) have a new tab-based layout that improves navigation between pages. The side menu has also changed quite a bit. You can still hide the side menu completely if you click on the LogDisplayPlatform logo.
...
The big new feature that comes with LogDisplayPlatform v5.0 is dashboard folders. Now you can organize your dashboards in folders,
...
A team is a new concept in LogDisplayPlatform v5. They are simply a group of users that can be used in the new permission system for dashboards and folders. Only an admin can create teams.
...
In previous versions of LogDisplayPlatform, you could only use the API for provisioning data sources and dashboards.
...
Data sources can now be setup using config files. These data sources are by default not editable from the LogDisplayPlatform GUI.
...
in sync with dashboards in LogDisplayPlatform's database. The dashboard provisioner has multiple advantages over the old
`[dashboard.json]` feature. Instead of storing the dashboard in memory we now insert the dashboard into the database,
which makes it possible to star them, use one as the home dashboard, set permissions and other features in LogDisplayPlatform that
