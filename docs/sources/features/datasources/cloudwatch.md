...
description = "Guide for using CloudWatch in LogDisplayPlatform"
keywords = ["logdisplayplatform", "cloudwatch", "guide"]
type = "docs"
...
# Using AWS CloudWatch in LogDisplayPlatform

LogDisplayPlatform ships with built in support for CloudWatch. You just have to add it as a data source and you will be ready to build dashboards for your CloudWatch metrics.
...
> NOTE: If at any moment you have issues with getting this datasource to work and LogDisplayPlatform is giving you undescriptive errors then don't
forget to check your log file (try looking in /var/log/logdisplayplatform/logdisplayplatform.log).
...
Currently all access to CloudWatch is done server side by the LogDisplayPlatform backend using the official AWS SDK. If you logdisplayplatform
...
LogDisplayPlatform needs permissions granted via IAM to be able to read CloudWatch metrics
and EC2 tags/instances. You can attach these permissions to IAM roles and
utilize LogDisplayPlatform's built-in support for assuming roles.
...
Create a file at `~/.aws/credentials`. That is the `HOME` path for user running logdisplayplatform-server.
    > NOTE: If you think you have the credentials file in the right place but it is still not working then you might try moving your .aws file to '/usr/share/logdisplayplatform/' and make sure your credentials file has at most 0644 permissions.
...
Note that the actual filtering takes place on Amazon's servers, not in LogDisplayPlatform.
...
it costs $0.01 per 1,000 GetMetricStatistics or ListMetrics requests. For each query LogDisplayPlatform will
issue a GetMetricStatistics request and every time you pick a dimension in the query editor
LogDisplayPlatform will issue a ListMetrics request.
