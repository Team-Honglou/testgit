...
func TestMiddlewareContext(t *testing.T) {
	...
	Convey("Given the logdisplayplatform middleware", t, func() {
		...
		middlewareScenario("Non api request should init session", func(sc *scenarioContext) {
			sc.fakeReq("GET", "/").exec()
			So(sc.resp.Header().Get("Set-Cookie"), ShouldContainSubstring, "logdisplayplatform_sess")
		})
		...
		middlewareScenario("When auth_proxy and ldap enabled call sync with ldap user", func(sc *scenarioContext) {
			...
			called := false
			syncLogDisplayPlatformUserWithLdapUser = func(query *m.LoginUserQuery) error {
				called = true
			...
			Convey("Should call syncLogDisplayPlatformUserWithLdapUser", func() {
...
func (sc *scenarioContext) fakeReq(method, url string) *scenarioContext {
	...
	if sc.context != nil {
		if sc.context.Session.ID() != "" {
			req.Header.Add("Cookie", "logdisplayplatform_sess="+sc.context.Session.ID()+";")
		}
	}
