+++
title = "Using MySQL in LogDisplayPlatform"
description = "Guide for using MySQL in LogDisplayPlatform"
keywords = ["logdisplayplatform", "mysql", "guide"]
type = "docs"
[menu.docs]
name = "MySQL"
parent = "datasources"
...
# Using MySQL in LogDisplayPlatform

> Only available in LogDisplayPlatform v4.3+.
>
> Starting from LogDisplayPlatform v5.1 you can name the time column *time* in addition to earlier supported *time_sec*. Usage of *time_sec*  will eventually be deprecated.

LogDisplayPlatform ships with a built-in MySQL data source plugin that allow you to query any visualize
...
the specified database & tables you want to query. LogDisplayPlatform does not validate that the query is safe. The query
...
```sql
 CREATE USER 'logdisplayplatformReader' IDENTIFIED BY 'password';
 GRANT SELECT ON mydatabase.mytable TO 'logdisplayplatformReader';
```
...
*$__timeGroup(dateColumn,'5m', 0)* | Same as above but with a fill parameter so missing points in that series will be added by logdisplayplatform and 0 will be used as value.
*$__timeGroup(dateColumn,'5m', NULL)* | Same as above but NULL will be used as value for missing points.
*$__timeGroup(dateColumn,'5m', previous)* | Same as above but the previous value in that series will be used as fill value if no value has been seen yet NULL will be used (only available in LogDisplayPlatform 5.3+).
*$__timeGroupAlias(dateColumn,'5m')* | Will be replaced identical to $__timeGroup but with an added column alias (only available in LogDisplayPlatform 5.3+).
*$__unixEpochFilter(dateColumn)* | Will be replaced by a time range filter using the specified column name with times represented as unix timestamp. For example, *dateColumn > 1494410783 AND dateColumn < 1494497183*
*$__unixEpochFrom()* | Will be replaced by the start of the currently active time selection as unix timestamp. For example, *1494410783*
*$__unixEpochTo()* | Will be replaced by the end of the currently active time selection as unix timestamp. For example, *1494497183*

We plan to add many more macros. If you have suggestions for what macros you would like to see, please [open an issue](https://github.com/logdisplayplatform/logdisplayplatform) in our GitHub repo.
...
If you return multiple value columns and a column named `metric` then this column is used as prefix for the series name (only available in LogDisplayPlatform 5.3+).
...
A query can returns multiple columns and LogDisplayPlatform will automatically create a list from them. For example, the query below will return a list with values from `hostname` and `hostname2`.
...
From LogDisplayPlatform 4.3.0 to 4.6.0, template variables are always quoted automatically so if it is a string value do not wrap them in quotes in where clauses.

From LogDisplayPlatform 4.7.0, template variable values are only quoted when the template variable is a `multi-value`.
...
LogDisplayPlatform automatically creates a quoted, comma-separated string for multi-value variables. For example: if `server01` and `server02` are selected then it will be formatted as: `'server01', 'server02'`. Do disable quoting, use the csv formatting option for variables:
