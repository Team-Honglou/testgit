+++
title = "Using InfluxDB in LogDisplayPlatform"
description = "Guide for using InfluxDB in LogDisplayPlatform"
keywords = ["logdisplayplatform", "influxdb", "guide"]
type = "docs"
...
# Using InfluxDB in LogDisplayPlatform

LogDisplayPlatform ships with very feature rich data source plugin for InfluxDB. Supporting a feature rich query editor, annotation and templating queries.
...
You can type in regex patterns for metric names or tag filter values, be sure to wrap the regex pattern in forward slashes (`/`). LogDisplayPlatform
