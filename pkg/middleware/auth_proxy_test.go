...
func TestAuthProxyWithLdapEnabled(t *testing.T) {
	Convey("When calling sync logdisplayplatform user with ldap user", t, func() {
		...
		Convey("When user logs in, call SyncUser", func() {
			...
			// act
			syncLogDisplayPlatformUserWithLdapUser(&m.LoginUserQuery{
				ReqContext: &ctx,
				Username:   "test",
			})
...
func (a *mockLdapAuthenticator) GetLogDisplayPlatformUserFor(ctx *m.ReqContext, ldapUser *login.LdapUserInfo) (*m.User, error) {
	return nil, nil
}
