+++
title = "Time Range"
keywords = ["logdisplayplatform", "dashboard", "documentation", "time range"]
type = "docs"
[menu.docs]
name = "Time Range"
...
LogDisplayPlatform provides numerous ways to manage the time ranges of the data being visualized, both at the Dashboard-level and the Panel-level.
...
Users often ask, [when will then be now](https://www.youtube.com/watch?v=VeZ9HhHU86o)? LogDisplayPlatform offers the ability to override the `now` value on a per dashboard basis. Most commonly, this feature is used to accommodate known delays in data aggregation to avoid null values.
