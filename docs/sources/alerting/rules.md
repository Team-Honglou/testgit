...
keywords = ["logdisplayplatform", "alerting", "guide", "rules"]
type = "docs"
[menu.docs]
name = "Engine & Rules"
parent = "alerting"
...
Alerting in LogDisplayPlatform allows you to attach rules to your dashboard panels. When you save the dashboard
LogDisplayPlatform will extract the alert rules into a separate alert rule storage and schedule them for evaluation.
...
The alert rules are evaluated in the LogDisplayPlatform backend in a scheduler and query execution engine that is part
of core LogDisplayPlatform. Only some data sources are supported right now. They include `Graphite`, `Prometheus`, `Elasticsearch`, `InfluxDB`, `OpenTSDB`, `MySQL`, `Postgres` and `Cloudwatch`.

> Alerting support for Elasticsearch is only available in LogDisplayPlatform v5.2 and above.

### Clustering

Currently alerting supports a limited form of high availability. Since v4.2.0 of LogDisplayPlatform, alert notifications are deduped when running multiple servers. This means all alerts are executed on every server but no duplicate alert notifications are sent due to the deduping logic. Proper load balancing of alerts will be introduced in the future.
...
What LogDisplayPlatform does not do currently is track alert rule state **per series**. This has implications that are detailed
...
So as you can see from the above scenario LogDisplayPlatform will not send out notifications when other series cause the alert
...
Alert state changes are recorded in the internal annotation table in LogDisplayPlatform's database. The state changes
...
Further troubleshooting can also be done by inspecting the logdisplayplatform-server log. If it's not an error or for some reason
the log does not say anything you can enable debug logging for some relevant components. This is done
in LogDisplayPlatform's ini config file.
...
If you want to log raw query sent to your TSDB and raw response in log you also have to set logdisplayplatform.ini option `app_mode` to
`development`.
