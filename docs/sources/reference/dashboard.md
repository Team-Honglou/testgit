+++
title = "JSON Model"
keywords = ["logdisplayplatform", "dashboard", "documentation", "json", "model"]
type = "docs"
[menu.docs]
name = "JSON Model"
...
A dashboard in LogDisplayPlatform is represented by a JSON object, which stores metadata of its dashboard. Dashboard metadata includes dashboard properties, metadata from panels, template variables, panel queries, etc.
...
| **schemaVersion** | version of the JSON schema (integer), incremented each time a LogDisplayPlatform update brings changes to said schema |
