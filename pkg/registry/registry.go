...
// are expected to forfill to be started within LogDisplayPlatform.
type Service interface {

	// Init is called by LogDisplayPlatform main process which gives the service
...
type BackgroundService interface {
	...
	// to subscribe to ctx.Done() so the service can be notified when LogDisplayPlatform shuts down.
