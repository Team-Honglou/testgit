...
var (
	...
	buildNumber           int      = 0
	binaries              []string = []string{"logdisplayplatform-server", "logdisplayplatform-cli"}
	isDev                 bool     = false
...
func main() {
	...
	flag.BoolVar(&enterprise, "enterprise", enterprise, "Build enterprise version of LogDisplayPlatform")
	...
	for _, cmd := range flag.Args() {
		...
		case "build-srv":
			clean()
			build("logdisplayplatform-server", "./pkg/cmd/logdisplayplatform-server", []string{})

		case "build-cli":
			clean()
			build("logdisplayplatform-cli", "./pkg/cmd/logdisplayplatform-cli", []string{})
...
func packageLogDisplayPlatform() {
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		"_amd64.deb":          "dist/logdisplayplatform_latest_amd64.deb",
		".x86_64.rpm":         "dist/logdisplayplatform-latest-1.x86_64.rpm",
		".linux-amd64.tar.gz": "dist/logdisplayplatform-latest.linux-x64.tar.gz",
	}
...
func createDebPackages() {
	...
	createPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/logdisplayplatform",
		binPath:                "/usr/sbin",
		configDir:              "/etc/logdisplayplatform",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/logdisplayplatform-server",
		initdScriptFilePath:    "/etc/init.d/logdisplayplatform-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/logdisplayplatform-server.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/logdisplayplatform-server",
		defaultFileSrc: "packaging/deb/default/logdisplayplatform-server",
		systemdFileSrc: "packaging/deb/systemd/logdisplayplatform-server.service",
...
func createRpmPackages() {
	...
	createPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/logdisplayplatform-server",
		initdScriptFilePath:    "/etc/init.d/logdisplayplatform-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/logdisplayplatform-server.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/logdisplayplatform-server",
		defaultFileSrc: "packaging/rpm/sysconfig/logdisplayplatform-server",
		systemdFileSrc: "packaging/rpm/systemd/logdisplayplatform-server.service",
...
func createPackage(options linuxPackageOptions) {
	packageRoot, _ := ioutil.TempDir("", "logdisplayplatform-linux-pack")
	...
	args := []string{
		"-s", "dir",
		"--description", "LogDisplayPlatform",
		"-C", packageRoot,
		"--url", "https://logdisplayplatform.com",
		"--maintainer", "contact@logdisplayplatform.com",
...
func setup() {
	...
	runPrint("go", "install", "-v", "./pkg/cmd/logdisplayplatform-server")
}
...
func clean() {
	...
	rmr("dist")
	rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/logdisplayplatform", goos, goarch)))
}
