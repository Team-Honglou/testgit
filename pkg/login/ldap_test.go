...
func TestLdapAuther(t *testing.T) {
	...
	Convey("When translating ldap user to logdisplayplatform user", t, func() {
		...
		ldapAutherScenario("Given wildcard group match", func(sc *scenarioContext) {
			...
			result, err := ldapAuther.GetLogDisplayPlatformUserFor(nil, &LdapUserInfo{})
			So(err, ShouldBeNil)
		...
		ldapAutherScenario("Given exact group match", func(sc *scenarioContext) {
			...
			result, err := ldapAuther.GetLogDisplayPlatformUserFor(nil, &LdapUserInfo{MemberOf: []string{"cn=users"}})
			So(err, ShouldBeNil)
			So(result, ShouldEqual, user1)
		})
		...
		ldapAutherScenario("Given no existing logdisplayplatform user", func(sc *scenarioContext) {
			ldapAuther := NewLdapAuthenticator(&LdapServerConf{
				LdapGroups: []*LdapGroupToOrgRole{
