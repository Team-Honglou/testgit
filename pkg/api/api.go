...
import (
	...
	"github.com/logdisplayplatform/logdisplayplatform/pkg/api/avatar"
	"github.com/logdisplayplatform/logdisplayplatform/pkg/api/dtos"
...
func (hs *HTTPServer) registerRoutes() {
	...
	reqLogDisplayPlatformAdmin := middleware.Auth(&middleware.AuthOptions{ReqSignedIn: true, ReqLogDisplayPlatformAdmin: true})
	...
	r.Get("/configuration", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/settings", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/users", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/users/create", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/users/edit/:id", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/orgs", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/orgs/edit/:id", reqLogDisplayPlatformAdmin, Index)
	r.Get("/admin/stats", reqLogDisplayPlatformAdmin, Index)
	...
	r.Group("/api", func(apiRoute RouteRegister) {
		...
		// search all orgs
		apiRoute.Get("/orgs", reqLogDisplayPlatformAdmin, wrap(SearchOrgs))
		...
		apiRoute.Get("/tsdb/testdata/gensql", reqLogDisplayPlatformAdmin, wrap(GenerateSQLTestData))
