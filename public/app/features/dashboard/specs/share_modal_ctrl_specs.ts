...
describe('ShareModalCtrl', function() {
  ...
  beforeEach(angularMocks.module('logdisplayplatform.controllers'));
  ...
  describe('shareUrl with current time range and panel', function() {
    ...
    it('should generate render url', function() {
      ctx.$location.$$absUrl = 'http://dashboards.logdisplayplatform.com/d/abcdefghi/my-dash';

      ctx.scope.panel = { id: 22 };

      ctx.scope.init();
      var base = 'http://dashboards.logdisplayplatform.com/render/d-solo/abcdefghi/my-dash';
    ...
    it('should generate render url for scripted dashboard', function() {
      ctx.$location.$$absUrl = 'http://dashboards.logdisplayplatform.com/dashboard/script/my-dash.js';

      ctx.scope.panel = { id: 22 };

      ctx.scope.init();
      var base = 'http://dashboards.logdisplayplatform.com/render/dashboard-solo/script/my-dash.js';
      var params = '?from=1000&to=2000&orgId=1&panelId=22&width=1000&height=500&tz=UTC';
      expect(ctx.scope.imageUrl).to.contain(base + params);
    });
