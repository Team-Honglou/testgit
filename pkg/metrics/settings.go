...
import (
	...
	"github.com/logdisplayplatform/logdisplayplatform/pkg/metrics/graphitebridge"
...
func (im *InternalMetricsService) parseGraphiteSettings() error {
	...
	bridgeCfg := &graphitebridge.Config{
		URL:             address,
		Prefix:          graphiteSection.Key("prefix").MustString("prod.logdisplayplatform.%(instance_name)s"),
	...
	if prefix == "" {
		prefix = "prod.logdisplayplatform.%(instance_name)s."
	}
