...
import (
	...
	"github.com/logdisplayplatform/logdisplayplatform-plugin-model/go/datasource"
...
func TestMappingRowValue(t *testing.T) {
	...
	stringRowValue, _ := dpw.mapRowValue(&datasource.RowValue{Kind: datasource.RowValue_TYPE_STRING, StringValue: "logdisplayplatform"})
	haveString, ok := stringRowValue.(string)
	if !ok || haveString != "logdisplayplatform" {
		t.Fatalf("Expected %s, was %s", "logdisplayplatform", haveString)
	}
