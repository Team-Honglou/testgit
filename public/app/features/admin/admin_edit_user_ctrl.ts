  ...
  constructor($scope, $routeParams, backendSrv, $location, navModelSrv) {
    ...
    $scope.getUser = function(id) {
      backendSrv.get('/api/users/' + id).then(function(user) {
        $scope.user = user;
        $scope.user_id = id;
        $scope.permissions.isLogDisplayPlatformAdmin = user.isLogDisplayPlatformAdmin;
      });
    };
...
angular.module('logdisplayplatform.controllers').controller('AdminEditUserCtrl', AdminEditUserCtrl);
