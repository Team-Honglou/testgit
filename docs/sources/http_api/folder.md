...
description = "LogDisplayPlatform Folder HTTP API"
keywords = ["logdisplayplatform", "http", "documentation", "api", "folder"]
...
The identifier (id) of a folder is an auto-incrementing numeric value and is only unique per LogDisplayPlatform install.

The unique identifier (uid) of a folder can be used for uniquely identify folders between multiple LogDisplayPlatform installs. It's automatically generated if not provided when creating a folder. The uid allows having consistent URL's for accessing folders and when syncing folders between multiple LogDisplayPlatform installs. This means that changing the title of a folder will not break any bookmarked links to that folder.
