...
export default function link(scope, elem, attrs, ctrl) {
  ...
  function addXAxis() {
    ...
    let logdisplayplatformTimeFormatter = ticksUtils.logdisplayplatformTimeFormat(ticks, timeRange.from, timeRange.to);
    let timeFormat;
    let dashboardTimeZone = ctrl.dashboard.getTimezone();
    if (dashboardTimeZone === 'utc') {
      timeFormat = d3.utcFormat(logdisplayplatformTimeFormatter);
    } else {
      timeFormat = d3.timeFormat(logdisplayplatformTimeFormatter);
    }
