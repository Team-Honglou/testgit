...
var apiUrl = flag.String("apiUrl", "https://logdisplayplatform.com/api", "api url")
var apiKey = flag.String("apiKey", "", "api key")
var version = ""
var versionRe = regexp.MustCompile(`logdisplayplatform-(.*)(\.|_)(arm64|armhfp|aarch64|armv7|darwin|linux|windows|x86_64)`)
var debVersionRe = regexp.MustCompile(`logdisplayplatform_(.*)_(arm64|armv7|armhf|amd64)\.deb`)
var builds = []build{}
...
func main() {
	...
	postRequest("/logdisplayplatform/versions", nightly, fmt.Sprintf("Create Release %s", nightly.Version))
	postRequest("/logdisplayplatform/versions/"+nightly.Version, nightly, fmt.Sprintf("Update Release %s", nightly.Version))

	for _, b := range nightly.Builds {
		postRequest(fmt.Sprintf("/logdisplayplatform/versions/%s/packages", nightly.Version), b, fmt.Sprintf("Create Build %s %s", b.Os, b.Arch))
		postRequest(fmt.Sprintf("/logdisplayplatform/versions/%s/packages/%s/%s", nightly.Version, b.Arch, b.Os), b, fmt.Sprintf("Update Build %s %s", b.Os, b.Arch))
	}
}
...
func mapPackage(path string, name string, shaBytes []byte) (build, error) {
	...
	return build{
		Os:     os,
		Arch:   arch,
		Url:    "https://s3-us-west-2.amazonaws.com/logdisplayplatform-releases/master/" + name,
