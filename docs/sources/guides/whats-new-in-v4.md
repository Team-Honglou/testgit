+++
title = "What's New in LogDisplayPlatform v4.0"
description = "Feature & improvement highlights for LogDisplayPlatform v4.0"
keywords = ["logdisplayplatform", "new", "documentation", "4.0"]
type = "docs"
[menu.docs]
name = "Version 4.0"
identifier = "v4.0"
...
# What's New in LogDisplayPlatform v4.0
...
Alerting is a really revolutionary feature for LogDisplayPlatform. It transforms LogDisplayPlatform from a
...
the right side of the graph. The rules will continually be evaluated by logdisplayplatform-server and
...
If you have configured an external image store in the logdisplayplatform.ini config file (s3, webdav, and azure_blob options available)
you can get very rich notifications with an image of the graph and the metric
values all included in the notification.

### Annotations

Alert state changes are recorded in a new annotation store that is built into LogDisplayPlatform. This store
currently only supports storing annotations in LogDisplayPlatform's own internal database (mysql, postgres or sqlite).
The LogDisplayPlatform annotation storage is currently only used for alert state changes but we hope to add the ability for users
to add graph comments in the form of annotations directly from within LogDisplayPlatform in a future release.
...
<div class="row">
  <div class="medium-6 columns">
    <p>
      LogDisplayPlatform is so often used on wall mounted TVs that we figured a clean TV mode would be
...
to another you will insert the panel and LogDisplayPlatform will automatically make room for it.
...
There are no breaking changes. Old dashboards and features should work the same. LogDisplayPlatform-server will automatically upgrade its db
schema on restart. It's advisable to do a backup of LogDisplayPlatform's database before updating.

If you are using plugins make sure to update your plugins as some might not work perfectly v4.

You can update plugins using logdisplayplatform-cli
