...
// The tests require a MySQL db named logdisplayplatform_ds_tests and a user/password logdisplayplatform/password
...
func InitMySQLTestDB(t *testing.T) *xorm.Engine {
	x, err := xorm.NewEngine(sqlutil.TestDB_Mysql.DriverName, strings.Replace(sqlutil.TestDB_Mysql.ConnStr, "/logdisplayplatform_tests", "/logdisplayplatform_ds_tests", 1))
	if err != nil {
