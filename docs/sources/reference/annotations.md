+++
title = "Annotations"
keywords = ["logdisplayplatform", "annotations", "documentation", "guide"]
type = "docs"
[menu.docs]
name = "Annotations"
...
LogDisplayPlatform v4.6+ comes with a native annotation store and the ability to add annotation events directly from the graph panel or via the [HTTP API]({{< relref "http_api/annotations.md" >}}).
...
You can create new annotation queries that fetch annotations from the native annotation store via the `-- LogDisplayPlatform --` data source and by setting *Filter by* to `Tags`. Specify at least
