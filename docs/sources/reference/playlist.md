+++
title = "Playlist"
keywords = ["logdisplayplatform", "dashboard", "documentation", "playlist"]
type = "docs"
[menu.docs]
...
Since LogDisplayPlatform automatically scales Dashboards to any resolution they're perfect for big screens!

## Creating a Playlist

{{< docs-imagebox img="/img/docs/v50/playlist.png" max-width="25rem" class="docs-image--right">}}

The Playlist feature can be accessed from LogDisplayPlatform's sidemenu, in the Dashboard submenu.

Click on "New Playlist" button to create a new playlist. Firstly, name your playlist and configure a time interval for LogDisplayPlatform to wait on a particular Dashboard before advancing to the next one on the Playlist.
...
For example, to open the first playlist on the LogDisplayPlatform Play site in kiosk mode: [http://play.logdisplayplatform.org/playlists/play/1?kiosk](http://play.logdisplayplatform.org/playlists/play/1?kiosk)
