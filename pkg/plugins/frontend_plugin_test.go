...
func TestFrontendPlugin(t *testing.T) {
	...
	Convey("When setting paths based on App on Windows", t, func() {
		setting.StaticRootPath = "c:\\logdisplayplatform\\public"

		fp := &FrontendPluginBase{
			PluginBase: PluginBase{
				PluginDir: "c:\\logdisplayplatform\\public\\app\\plugins\\app\\testdata\\datasources\\datasource",
				BaseUrl:   "fpbase",
			},
		}
		app := &AppPlugin{
			FrontendPluginBase: FrontendPluginBase{
				PluginBase: PluginBase{
					PluginDir: "c:\\logdisplayplatform\\public\\app\\plugins\\app\\testdata",
					Id:        "testdata",
