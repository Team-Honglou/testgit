...
var logdisplayplatformComProxyTransport = &http.Transport{
...
func ReverseProxyGnetReq(proxyPath string) *httputil.ReverseProxy {
	url, _ := url.Parse(setting.LogDisplayPlatformComUrl)
...
func ProxyGnetRequest(c *m.ReqContext) {
	...
	proxy.Transport = logdisplayplatformComProxyTransport
