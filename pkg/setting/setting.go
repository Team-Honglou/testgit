...
var (
	...
	// LogDisplayPlatform.NET URL
	LogDisplayPlatformComUrl string
...
func loadConfiguration(args *CommandLineArgs) (*ini.File, error) {
	...
	if _, err := os.Stat(defaultConfigFile); os.IsNotExist(err) {
		fmt.Println("LogDisplayPlatform-server Init Failed: Could not find config defaults, make sure homepath command line parameter is set or working directory is homepath")
		os.Exit(1)
	}
...
func (cfg *Cfg) Load(args *CommandLineArgs) error {
	...
	// check old key  name
	LogDisplayPlatformComUrl = iniFile.Section("logdisplayplatform_net").Key("url").MustString("")
	if LogDisplayPlatformComUrl == "" {
		LogDisplayPlatformComUrl = iniFile.Section("logdisplayplatform_com").Key("url").MustString("https://logdisplayplatform.com")
	}
...
func (cfg *Cfg) readSessionConfig() {
	...
	SessionOptions.CookieName = sec.Key("cookie_name").MustString("logdisplayplatform_sess")
