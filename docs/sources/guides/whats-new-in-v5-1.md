+++
title = "What's New in LogDisplayPlatform v5.1"
description = "Feature & improvement highlights for LogDisplayPlatform v5.1"
keywords = ["logdisplayplatform", "new", "documentation", "5.1"]
type = "docs"
[menu.docs]
name = "Version 5.1"
identifier = "v5.1"
...
# What's New in LogDisplayPlatform v5.1

LogDisplayPlatform v5.1 brings new features, many enhancements and bug fixes. This article will detail the major new features and enhancements.
...
In LogDisplayPlatform v5.0 we introduced a new scrollbar component. Unfortunately this introduced a lot of issues and in some scenarios removed
the native scrolling functionality. LogDisplayPlatform v5.1 ships with a native scrollbar for all pages together with a scrollbar component for
...
make the LogDisplayPlatform user experience much better!

## Improved docker image (breaking change)

LogDisplayPlatform v5.1 brings an improved official docker image which should make it easier to run and use the LogDisplayPlatform docker image and at the same time give more control to the user how to use/run it.

We've switched the id of the logdisplayplatform user running LogDisplayPlatform inside a docker container. Unfortunately this means that files created prior to 5.1 won't have the correct permissions for later versions and thereby this introduces a breaking change.
We made this change so that it would be easier for you to control what user LogDisplayPlatform is executed as (see examples below).
...
LogDisplayPlatform v5.1 now ships with a built-in Microsoft SQL Server (MSSQL) data source plugin that allows you to query and visualize data from any
...
that data and define alert rules on it like with any of LogDisplayPlatform's other core datasources.

Please read [Using Microsoft SQL Server in LogDisplayPlatform documentation](/features/datasources/mssql/) for more detailed information on how to get started and use it.
...
In the table below you can see some examples and you can find all different options in the [Variables documentation](http://docs.logdisplayplatform.org/reference/templating/#advanced-formatting-options).
...
LogDisplayPlatform v5.1 brings an improved workflow for provisioned dashboards:
