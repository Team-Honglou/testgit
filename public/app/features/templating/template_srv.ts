...
export class TemplateSrv {
  ...
  private index = {};
  private logdisplayplatformVariables = {};
  private builtIns = {};
  ...
  setLogDisplayPlatformVariable(name, value) {
    this.logdisplayplatformVariables[name] = value;
  }
  ...
  replace(target, scopedVars?, format?) {
    ...
    return target.replace(this.regex, (match, var1, var2, fmt2, var3, fmt3) => {
      ...
      systemValue = this.logdisplayplatformVariables[variable.current.value];
      if (systemValue) {
  ...
  replaceWithText(target, scopedVars) {
    ...
    return target.replace(this.regex, (match, var1, var2, fmt2, var3) => {
      ...
      return this.logdisplayplatformVariables[variable.current.value] || variable.current.text;
    });
  }
