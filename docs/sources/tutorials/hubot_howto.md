+++
title = "How To integrate Hubot and LogDisplayPlatform"
type = "docs"
keywords = ["logdisplayplatform", "tutorials", "hubot", "slack", "hipchat", "setup", "install", "config"]
[menu.docs]
parent = "tutorials"
weight = 10
+++

# How to integrate Hubot with LogDisplayPlatform

LogDisplayPlatform 2.0 shipped with a great feature that enables it to render any graph or panel to a PNG image.
No matter what data source you are using, the PNG image of the Graph will look the same
as it does in your browser.

This guide will show you how to install and configure the [Hubot-LogDisplayPlatform](https://github.com/stephenyeargin/hubot-logdisplayplatform)
...
> *Amazon S3 Required*: The hubot-logdisplayplatform script will upload the rendered graphs to Amazon S3. This
> is so Hipchat and Slack can show them reliably (they require the image to be publicly available).

<div class="text-center">
  <img src="/img/docs/tutorials/hubot_logdisplayplatform.png" class="center"></a>
</div>
...
## Install Hubot-LogDisplayPlatform script

In your Hubot project repo install the LogDisplayPlatform plugin using `npm`:
```bash
npm install hubot-logdisplayplatform --save
```
Edit the file external-scripts.json, and add hubot-logdisplayplatform to the list of plugins.

```json
[
"hubot-pugme",
"hubot-shipit",
"hubot-logdisplayplatform"
]
```

## Configure

The `hubot-logdisplayplatform` plugin requires a number of environment variables to be set in order to work properly.

```bash
export HUBOT_GRAFANA_HOST=http://play.logdisplayplatform.org
...
### LogDisplayPlatform server side rendering

The hubot plugin will take advantage of the LogDisplayPlatform server side rendering feature that can
render any panel on the server using phantomjs. LogDisplayPlatform ships with a phantomjs binary (linux only).
...
### LogDisplayPlatform API Key

{{< docs-imagebox img="/img/docs/v2/orgdropdown_api_keys.png" max-width="150px" class="docs-image--right">}}

You need to set the environment variable `HUBOT_GRAFANA_API_KEY` to a LogDisplayPlatform API Key.
...
to publicly available. By specifying the `S3` options the hubot-logdisplayplatform script will publish the rendered
...
<div class="text-center">
  Using the alias:<br>
  <img src="/img/docs/tutorials/hubot_logdisplayplatform2.png" class="center"></a>
</div>

## Summary

LogDisplayPlatform is going to ship with integrated Slack and Hipchat features some day but you do
not have to wait for that. LogDisplayPlatform 2 shipped with a very clever server side rendering feature
that can render any panel to a png using phantomjs. The hubot plugin for LogDisplayPlatform is something
