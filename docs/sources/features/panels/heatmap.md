...
keywords = ["logdisplayplatform", "heatmap", "panel", "documentation"]
type = "docs"
[menu.docs]
name = "Heatmap"
parent = "panels"
...
> New panel only available in LogDisplayPlatform v4.3+
...
------------ | -------------
*Time series* | LogDisplayPlatform does the bucketing by going through all time series values. The bucket sizes & intervals will be determined using the Buckets options.
*Time series buckets* | Each time series already represents a Y-Axis bucket. The time series name (alias) needs to be a numeric value representing the upper or lower interval for the bucket. LogDisplayPlatform does no bucketing so the bucket size options are hidden.
...
The Bucket count & size options are used by LogDisplayPlatform to calculate how big each cell in the heatmap is. You can
...
that LogDisplayPlatform performs may be done on already aggregated and averaged data. To get more accurate heatmaps it is better
