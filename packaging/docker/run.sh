...
if [ $PERMISSIONS_OK -eq 1 ]; then
    echo "You may have issues with file permissions, more information here: http://docs.logdisplayplatform.org/installation/docker/#migration-from-a-previous-version-of-the-docker-container-to-5-1-or-later"
fi
...
if [ ! -z "${GF_INSTALL_PLUGINS}" ]; then
  ...
  for plugin in ${GF_INSTALL_PLUGINS}; do
    IFS=$OLDIFS
    logdisplayplatform-cli --pluginsDir "${GF_PATHS_PLUGINS}" plugins install ${plugin}
  done
fi

exec logdisplayplatform-server                                         \
