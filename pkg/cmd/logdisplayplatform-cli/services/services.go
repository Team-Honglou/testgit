...
import (
	...
	m "github.com/logdisplayplatform/logdisplayplatform/pkg/cmd/logdisplayplatform-cli/models"
)

var (
	IoHelper       m.IoUtil = IoUtilImp{}
	HttpClient     http.Client
	logdisplayplatformVersion string
)

func Init(version string, skipTLSVerify bool) {
	logdisplayplatformVersion = version
...
func sendRequest(repoUrl string, subPaths ...string) ([]byte, error) {
	...
	req.Header.Set("logdisplayplatform-version", logdisplayplatformVersion)
	req.Header.Set("logdisplayplatform-os", runtime.GOOS)
	req.Header.Set("logdisplayplatform-arch", runtime.GOARCH)
	req.Header.Set("User-Agent", "logdisplayplatform "+logdisplayplatformVersion)
