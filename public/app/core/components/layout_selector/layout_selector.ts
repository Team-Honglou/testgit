...
export class LayoutSelectorCtrl {
  ...
  constructor(private $rootScope) {
    this.mode = store.get('logdisplayplatform.list.layout.mode') || 'grid';
  }

  listView() {
    this.mode = 'list';
    store.set('logdisplayplatform.list.layout.mode', 'list');
    this.$rootScope.appEvent('layout-mode-changed', 'list');
  }

  gridView() {
    this.mode = 'grid';
    store.set('logdisplayplatform.list.layout.mode', 'grid');
...
export function layoutMode($rootScope) {
  return {
    ...
    link: function(scope, elem) {
      var layout = store.get('logdisplayplatform.list.layout.mode') || 'grid';
