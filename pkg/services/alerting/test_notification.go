...
func createTestEvalContext(cmd *NotificationTestCommand) *EvalContext {
	testRule := &Rule{
		...
		Message:     "Someone is testing the alert notification within logdisplayplatform.",
		State:       m.AlertStateAlerting,
	}

	ctx := NewEvalContext(context.Background(), testRule)
	if cmd.Settings.Get("uploadImage").MustBool(true) {
		ctx.ImagePublicUrl = "http://logdisplayplatform.org/assets/img/blog/mixed_styles.png"
	}
	ctx.IsTestRun = true
	ctx.Firing = true
