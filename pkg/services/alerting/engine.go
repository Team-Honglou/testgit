...
import (
	...
	"github.com/logdisplayplatform/logdisplayplatform/pkg/services/rendering"
...
func (e *AlertingService) alertingTicker(logdisplayplatformCtx context.Context) error {
	defer func() {
	...
	for {
		select {
		case <-logdisplayplatformCtx.Done():
			return logdisplayplatformCtx.Err()
...
func (e *AlertingService) runJobDispatcher(logdisplayplatformCtx context.Context) error {
	dispatcherGroup, alertCtx := errgroup.WithContext(logdisplayplatformCtx)
...
func (e *AlertingService) processJobWithRetry(logdisplayplatformCtx context.Context, job *Job) error {
	defer func() {
		if err := recover(); err != nil {
	...
	for {
		select {
		case <-logdisplayplatformCtx.Done():
			// In case logdisplayplatform server context is cancel, let a chance to job processing
			...
			case <-unfinishedWorkTimer.C:
				return e.endJob(logdisplayplatformCtx.Err(), cancelChan, job)
			case <-attemptChan:
