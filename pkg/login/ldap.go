...
type ILdapAuther interface {
	...
	GetLogDisplayPlatformUserFor(ctx *m.ReqContext, ldapUser *LdapUserInfo) (*m.User, error)
}
...
func (a *ldapAuther) Login(query *m.LoginUserQuery) error {
	...
	logdisplayplatformUser, err := a.GetLogDisplayPlatformUserFor(query.ReqContext, ldapUser)
	if err != nil {
		return err
	}

	query.User = logdisplayplatformUser
	return nil
}
...
func (a *ldapAuther) GetLogDisplayPlatformUserFor(ctx *m.ReqContext, ldapUser *LdapUserInfo) (*m.User, error) {
