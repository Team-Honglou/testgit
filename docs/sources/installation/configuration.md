...
keywords = ["logdisplayplatform", "configuration", "documentation"]
type = "docs"
[menu.docs]
name = "Configuration"
identifier = "config"
...
A common problem is forgetting to uncomment a line in the `custom.ini` (or `logdisplayplatform.ini`) file which causes the configuration option to be ignored.
...
> **Note.** If you have installed LogDisplayPlatform using the `deb` or `rpm`
> packages, then your configuration file is located at
> `/etc/logdisplayplatform/logdisplayplatform.ini`. This path is specified in the LogDisplayPlatform
...
Set the name of the logdisplayplatform-server instance. Used in logging and internal metrics and in
clustering info. Defaults to: `${HOSTNAME}`, which will be replaced with
environment variable `HOSTNAME`, if that is empty or does not exist LogDisplayPlatform will try to use
...
Path to where LogDisplayPlatform will store logs. This path is usually specified via
...
Directory where logdisplayplatform will automatically scan and look for plugins

### provisioning

Folder that contains [provisioning](/administration/provisioning) config files that logdisplayplatform will apply on startup. Dashboards will be reloaded when the json files changes
...
either give the LogDisplayPlatform binary permission for example:

```bash
$ sudo setcap 'cap_net_bind_service=+ep' /usr/sbin/logdisplayplatform-server
```

Or redirect port 80 to the LogDisplayPlatform port using:

```bash
$ sudo iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 3000
```

Another way is put a webserver like Nginx or Apache in front of LogDisplayPlatform and have them proxy requests to LogDisplayPlatform.

### protocol

`http` or `https`

> **Note** LogDisplayPlatform versions earlier than 3.0 are vulnerable to [POODLE](https://en.wikipedia.org/wiki/POODLE). So we strongly recommend to upgrade to 3.x or use a reverse proxy for ssl termination.
...
This is the full URL used to access LogDisplayPlatform from a web browser. This is
...
> in front of LogDisplayPlatform that exposes it through a subpath. In that
> case add the subpath to the end of this URL setting.

### static_root_path

The path to the directory where the front end files (HTML, JS, and CSS
files). Default to `public` which is why the LogDisplayPlatform binary needs to be
...
Set to true for LogDisplayPlatform to log all HTTP requests (not just errors). These are logged as Info level events
to logdisplayplatform log.
<hr />
...
LogDisplayPlatform needs a database to store users and dashboards (and other
things). By default it is configured to use `sqlite3` which is an
embedded database (included in the main LogDisplayPlatform binary).
...
The name of the LogDisplayPlatform database. Leave it set to `logdisplayplatform` or some
other name.
...
The name of the default LogDisplayPlatform admin user (who has full permissions).
Defaults to `admin`.

### admin_password

The password of the default LogDisplayPlatform admin. Set once on first-run.  Defaults to `admin`.
...
users from the [LogDisplayPlatform Admin Pages](../../reference/admin)
...
you change your organization name in the LogDisplayPlatform UI this setting needs
...
```bash
http://<my_logdisplayplatform_server_name_or_ip>:<logdisplayplatform_server_port>/login/github
```

This callback URL must match the full HTTP address that you use in your
browser to access LogDisplayPlatform, but with the prefix path of `/login/github`.
When the GitHub OAuth application is created you will get a Client ID and a
Client Secret. Specify these in the LogDisplayPlatform configuration file. For
example:
...
Restart the LogDisplayPlatform back-end. You should now see a GitHub login button
...
LogDisplayPlatform instance. For example:
...
your LogDisplayPlatform instance. For example
...
3. Enter the following:
   ...
   - Authorized Javascript Origins: https://logdisplayplatform.mycompany.com
   - Authorized Redirect URLs: https://logdisplayplatform.mycompany.com/login/google

   Replace https://logdisplayplatform.mycompany.com with the URL of your LogDisplayPlatform instance.

4. Click Create

5. Copy the Client ID and Client Secret from the 'OAuth Client' modal

Specify the Client ID and Secret in the LogDisplayPlatform configuration file. For example:
...
Restart the LogDisplayPlatform back-end. You should now see a Google login button
...
This callback URL must match the full HTTP address that you use in your
browser to access LogDisplayPlatform, but with the prefix path of `/login/generic_oauth`.
...
First set up LogDisplayPlatform as an OpenId client "webapplication" in Okta. Then set the Base URIs to `https://<logdisplayplatform domain>/` and set the Login redirect URIs to `https://<logdisplayplatform domain>/login/generic_oauth`.
...
1.  Create a new Custom Connector with the following settings:
    ...
    - Redirect URI: `https://<logdisplayplatform domain>/login/generic_oauth`
    - Signing Algorithm: RS256
    - Login URL: `https://<logdisplayplatform domain>/login/generic_oauth`

    then:
2.  Add an App to the LogDisplayPlatform Connector:
    - Display Name: LogDisplayPlatform

    then:
3.  Under the SSO tab on the LogDisplayPlatform App details page you'll find the Client ID and Client Secret.

    Your OneLogin Domain will match the url you use to access OneLogin.

    Configure LogDisplayPlatform as follows:
...
2.  Go to the Settings tab and set:
    - Allowed Callback URLs: `https://<logdisplayplatform domain>/login/generic_oauth`

3. Click Save Changes, then use the values at the top of the page to configure LogDisplayPlatform:
...
3.  Click "App Registrations" and add a new application registration:
    ...
    - Sign-on URL: `https://<logdisplayplatform domain>/login/generic_oauth`
...
6.  Click "Settings", then click "Keys" and add a new entry under Passwords
    - Key Description: LogDisplayPlatform OAuth
    - Duration: Never Expires

7.  Click Save then copy the key value, this will be the OAuth client secret.

8.  Configure LogDisplayPlatform as follows:
...
### config_file
Path to the LDAP specific configuration file (default: `/etc/logdisplayplatform/ldap.toml`)

### allow_sign_up

Allow sign up should almost always be true (default) to allow new LogDisplayPlatform users to be created (if ldap authentication is ok). If set to
false only pre-existing LogDisplayPlatform users will be able to login (if ldap authentication is ok).
...
Set to `true` to enable auto sign up of users who do not exist in LogDisplayPlatform DB. Defaults to `true`.
...
The name of the LogDisplayPlatform session cookie.

### cookie_secure

Set to true if you host LogDisplayPlatform behind HTTPS only. Defaults to `false`.
...
When enabled LogDisplayPlatform will send anonymous usage statistics to
`stats.logdisplayplatform.org`. No IP addresses are being tracked, only simple counters to
...
If you want to track LogDisplayPlatform usage via Google analytics specify *your* Universal
...
### from_address
Address used when sending out emails, defaults to `admin@logdisplayplatform.localhost`

### from_name
Name to be used when sending out emails, defaults to `LogDisplayPlatform`
...
## [metrics.graphite]
Include this section if you want to send internal LogDisplayPlatform metrics to Graphite.

### address
Format `<Hostname or ip>`:port

### prefix
Graphite metric prefix. Defaults to `prod.logdisplayplatform.%(instance_name)s.`
...
Set root url to a LogDisplayPlatform instance where you want to publish external snapshots (defaults to https://snapshots-origin.raintank.io)
...
### provider
You can choose between (s3, webdav, gcs, azure_blob, local). If left empty LogDisplayPlatform will ignore the upload action.

## [external_image_storage.s3]

### bucket
Bucket name for S3. e.g. logdisplayplatform.snapshot
...
- http://logdisplayplatform.s3.amazonaws.com/
- https://logdisplayplatform.s3-ap-southeast-2.amazonaws.com/
