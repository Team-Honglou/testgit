+++
title = "Using Graphite in LogDisplayPlatform"
description = "Guide for using graphite in LogDisplayPlatform"
keywords = ["logdisplayplatform", "graphite", "guide"]
type = "docs"
...
# Using Graphite in LogDisplayPlatform

LogDisplayPlatform has an advanced Graphite query editor that lets you quickly navigate the metric space, add functions,
...
All requests will be made from the browser to LogDisplayPlatform backend/server which in turn will forward the requests to the data source and by that circumvent possible Cross-Origin Resource Sharing (CORS) requirements. The URL needs to be accessible from the logdisplayplatform backend/server if you select this access mode.
...
> client side by LogDisplayPlatform. And depending on your consolidation function only one or two can be correct at the same time.
...
Graphite 1.1 introduced tags and LogDisplayPlatform added support for Graphite queries with tags in version 5.0. To create a variable using tag values, then you need to use the LogDisplayPlatform functions `tags` and `tag_values`.
...
Multi-value variables in tag queries use the advanced formatting syntax introduced in LogDisplayPlatform 5.0 for variables: `{var:regex}`. Non-tag queries will use the default glob formatting for multi-value variables.
