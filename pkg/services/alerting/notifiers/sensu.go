...
func (this *SensuNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	// else we fallback and use the logdisplayplatform ruleID.
	if this.Source != "" {
		bodyJSON.Set("source", this.Source)
	} else {
		bodyJSON.Set("source", "logdisplayplatform_rule_"+strconv.FormatInt(evalContext.Rule.Id, 10))
	}
	// Finally, sensu expects an output
	// We set it to a default output
	bodyJSON.Set("output", "LogDisplayPlatform Metric Condition Met")
