...
The LogDisplayPlatform team reviews all plugins that are published on LogDisplayPlatform.com. There are two areas we review, the metadata for the plugin and the plugin functionality.

## Metadata

The plugin metadata consists of a `plugin.json` file and the README.md file. These `plugin.json` file is used by LogDisplayPlatform to load the plugin and the README.md file is shown in the plugins section of LogDisplayPlatform and the plugins section of LogDisplayPlatform.com.

### README.md

The README.md file is shown on the plugins page in LogDisplayPlatform and the plugin page on LogDisplayPlatform.com. There are some differences between the GitHub markdown and the markdown allowed in LogDisplayPlatform/LogDisplayPlatform.com:

- Cannot contain inline HTML.
- Any image links should be absolute links. For example: https://raw.githubusercontent.com/logdisplayplatform/azure-monitor-datasource/master/dist/img/logdisplayplatform_cloud_install.png
...
The `plugin.json` file is the same concept as the `package.json` file for an npm package. When the LogDisplayPlatform server starts it will scan the plugin folders (all folders in the data/plugins subfolder) and load every folder that contains a `plugin.json` file unless the folder contains a subfolder named `dist`. In that case, the LogDisplayPlatform server will load the `dist` folder instead.
...
```json
{
  ...
  "info": {
    "description": "Clock panel for logdisplayplatform",
    "author": {
      "name": "Author Name",
  ...
  "dependencies": {
    "logdisplayplatformVersion": "3.x.x",
    "plugins": [ ]
  }
}
```

- The convention for the plugin id is [github username/org]-[plugin name]-[datasource|app|panel] and it has to be unique. Although if org and plugin name are the same then [plugin name]-[datasource|app|panel] is also valid. The org **cannot** be `logdisplayplatform` unless it is a plugin created by the LogDisplayPlatform core team.
...
The full file format for the `plugin.json` file is described [here](http://docs.logdisplayplatform.org/plugins/developing/plugin.json/).
...
For the HTML on editor tabs, we recommend using the inbuilt LogDisplayPlatform styles rather than defining your own. This makes plugins feel like a more natural part of LogDisplayPlatform. If done correctly, the html will also be responsive and adapt to smaller screens. The `gf-form` css classes should be used for labels and inputs.
...
A basic guide for data sources can be found [here](http://docs.logdisplayplatform.org/plugins/developing/datasources/).
...
If possible, any passwords or secrets should be be saved in the `secureJsonData` blob. To encrypt sensitive data, the LogDisplayPlatform server's proxy feature must be used. The LogDisplayPlatform server has support for token authentication (OAuth) and HTTP Header authentication. If the calls have to be sent directly from the browser to a third-party API then this will not be possible and sensitive data will not be encrypted.
...
- Should handle errors. There are two main ways to do this:
  - use the notification system in LogDisplayPlatform to show a toaster popup with the error message. Example [here](https://github.com/alexanderzobnin/logdisplayplatform-zabbix/blob/fdbbba2fb03f5f2a4b3b0715415e09d5a4cf6cde/src/panel-triggers/triggers_panel_ctrl.js#L467-L471).
  - provide an error notification in the query editor like the MySQL/Postgres data sources do. Example code in the `query_ctrl`  [here](https://github.com/logdisplayplatform/azure-monitor-datasource/blob/b184d077f082a69f962120ef0d1f8296a0d46f03/src/query_ctrl.ts#L36-L51) and in the [html](https://github.com/logdisplayplatform/azure-monitor-datasource/blob/b184d077f082a69f962120ef0d1f8296a0d46f03/src/partials/query.editor.html#L190-L193).
