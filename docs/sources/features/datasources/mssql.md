+++
title = "Using Microsoft SQL Server in LogDisplayPlatform"
description = "Guide for using Microsoft SQL Server in LogDisplayPlatform"
keywords = ["logdisplayplatform", "MSSQL", "Microsoft", "SQL", "guide", "Azure SQL Database"]
type = "docs"
[menu.docs]
...
# Using Microsoft SQL Server in LogDisplayPlatform

> Only available in LogDisplayPlatform v5.1+.

LogDisplayPlatform ships with a built-in Microsoft SQL Server (MSSQL) data source plugin that allows you to query and visualize data from any Microsoft SQL Server 2005 or newer, including Microsoft Azure SQL Database.
...
```sql
 CREATE USER logdisplayplatformreader WITH PASSWORD 'password'
 GRANT SELECT ON dbo.YourTable3 TO logdisplayplatformreader
```
...
A query can return multiple columns and LogDisplayPlatform will automatically create a list from them. For example, the query below will return a list with values from `hostname` and `hostname2`.
...
>
> From LogDisplayPlatform 5.0.0, template variable values are only quoted when the template variable is a `multi-value`.
